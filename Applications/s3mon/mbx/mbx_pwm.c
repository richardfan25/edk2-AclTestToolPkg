...
int mbx_pwm_getFreq(uint8_t did, uint16_t *freq)
{
    ...
    if (!mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_RD_PWM_FREQ)) return _MBEr_Fail;  //set CMD 0x36
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;            							//wait cmd clear
...
int mbx_pwm_setFreq(uint8_t did, uint16_t freq)
{
    ...
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;            								//wait cmd clear
    if (!mbox.writeBoxBuf(MBX_OFFSET_PARA, did)) return _MBEr_Fail;     				//set PAPA 0, device id
    if (!mbox.writeBoxBuf(MBX_OFFSET_DATA, (uint8_t)(freq >> 8))) return _MBEr_Fail; 	//set Dat00
    if (!mbox.writeBoxBuf(MBX_OFFSET_DATA + 1, (uint8_t)freq)) return _MBEr_Fail;   	//set Dat01
    if (!mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_WR_PWM_FREQ)) return _MBEr_Fail;  	//set CMD 0x32 write gpio command
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;            								//wait cmd clear
    if (!mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp)) return _MBEr_Fail;  					//read status
    if (u8tmp != _MBEr_Success) return u8tmp;                  							//fail
...
int mbx_pwm_getPolarity(uint8_t did, uint8_t *pol)
{
    ...
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;            							//wait cmd clear
    if (!mbox.writeBoxBuf(MBX_OFFSET_PARA, did)) return _MBEr_Fail;     			//set PAPA 0, device id
    if (!mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_RD_PWM_POL)) return _MBEr_Fail;   //set CMD 0x37
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;            							//wait cmd clear
    if (!mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp)) return _MBEr_Fail;  				//read status
    if (u8tmp != _MBEr_Success) return u8tmp;             							//fail
    if (!mbox.readBoxBuf(MBX_OFFSET_DATA,pol)) return _MBEr_Fail;     				//set Dat00
...
int mbx_pwm_setPolarity(uint8_t did, uint8_t pol)
{
    ...
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;            				//wait cmd clear
    if (!mbox.writeBoxBuf(MBX_OFFSET_PARA, did)) return _MBEr_Fail;     //set PAPA 0, device id
    if (!mbox.writeBoxBuf(MBX_OFFSET_DATA, pol)) return _MBEr_Fail;     //set Dat00
    if (!mbox.writeBoxBuf(MBX_OFFSET_CMD, 0x33)) return _MBEr_Fail;    	//set CMD 0x33 write gpio command
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;            				//wait cmd clear
    if (!mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp)) return _MBEr_Fail;  	//read status
    if (u8tmp != _MBEr_Success) return u8tmp;                  	//fail
