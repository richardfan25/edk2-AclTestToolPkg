					...
					uint8_t *wdata, uint8_t *rdata, uint8_t wlen, uint16_t *rlen)
{
    ...
    mbox.clearBoxBuf();
    if (!mbox.waitBoxBusy()) return SMB_ST_ERR_UNKNOWN;               				//wait cmd clear
    if (!mbox.writeBoxBuf(MBX_OFFSET_PARA, did)) return SMB_ST_ERR_UNKNOWN;      	//set PAPA 0, device id
    if (!mbox.writeBoxBuf(MBX_OFFSET_DATA, address)) return SMB_ST_ERR_UNKNOWN;  	//set Dat00
    if (!mbox.writeBoxBuf(MBX_OFFSET_DATA + 1, smbcmd)) return SMB_ST_ERR_UNKNOWN; 	//set Dat01
    ...
    case MBXI2C_WriteReadBuf:
        ...
        if (!mbox.writeBoxBuf(MBX_OFFSET_DATA + 2, u8tmp)) return SMB_ST_ERR_UNKNOWN;	//set Dat02, read count
        ...
        if (!mbox.writeBoxBuf(MBX_OFFSET_DATA + 3, u8tmp)) return SMB_ST_ERR_UNKNOWN;	//set Dat03, write count
    ...
    //start transaction
    if (!mbox.writeBoxBuf(MBX_OFFSET_CMD, icmd)) return SMB_ST_ERR_UNKNOWN;            	//set CMD
    if (!mbox.waitBoxBusy()) return SMB_ST_ERR_UNKNOWN;            						//wait cmd clear
    if (!mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp)) return (SMB_ST_DONE|SMB_ST_ERR_UNKNOWN);  //read status
...
int mbx_smb_getFreq(uint8_t did, uint16_t *freq)
{
    ...
    if (!mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_RD_SMB_FREQ)) return _MBEr_Fail;      //set cmd 0x34
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;                							//wait cmd clear
    if (!mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp)) return _MBEr_Fail;      				//read status
    if (u8tmp != _MBEr_Success) return u8tmp;                      						//fail
    if (!mbox.readBoxBuf(MBX_OFFSET_DATA, &u8tmp)) return _MBEr_Fail;      				//read dat0
    switch (u8tmp)
    {
    case 0x00:
        if (!mbox.readBoxBuf(MBX_OFFSET_DATA + 1, &u8tmp)) return _MBEr_Fail;      		//read dat1
        *freq = (uint16_t)u8tmp;
        break;
    case 0x01:
        *freq = 50;
...
int mbx_smb_setFreq(uint8_t did, uint16_t freq)
{
    ...
    {
    case 50:
        if (!mbox.writeBoxBuf(MBX_OFFSET_DATA, 0x01)) return _MBEr_Fail;    //set DAT00
        break;
    case 100:
        if (!mbox.writeBoxBuf(MBX_OFFSET_DATA, 0x02)) return _MBEr_Fail;    //set DAT00
        break;
    case 400:
        if (!mbox.writeBoxBuf(MBX_OFFSET_DATA, 0x03)) return _MBEr_Fail;    //set DAT00
        break;
    default:
        if (!mbox.writeBoxBuf(MBX_OFFSET_DATA, 0x00)) return _MBEr_Fail;    //set DAT00
        if (!mbox.writeBoxBuf(MBX_OFFSET_DATA + 1, (uint8_t)freq)) return _MBEr_Fail; //set DAT01
        break;
    }
    if (!mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_WR_SMB_FREQ)) return _MBEr_Fail;	//set cmd 0x35
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;                				//wait cmd clear
    if (!mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp)) return _MBEr_Fail;      	//read status
    if (u8tmp != _MBEr_Success) return u8tmp;                      			//fail
    return _MBEr_Success;
