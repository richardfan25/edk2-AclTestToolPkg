...
int mbx_tml_updateDevice(void)
{
	...
	for(tml_no = 0; tml_no < mbx_tml.dev_cnt; tml_no++)
	{
		...
		if (!mbox.writeBoxBuf(MBX_OFFSET_PARA, tml_no)) return _MBEr_Fail;       			//set PAPA, zone number
		if (!mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_RD_THERMZONE)) return _MBEr_Fail; //set Cmd 0x42
		if (!mbox.waitBoxBusy()) return _MBEr_Fail;               						//wait cmd clear
		if (!mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp)) return _MBEr_Fail;     			//read status
		if (u8tmp != _MBEr_Success) return u8tmp;                 //fail

		if (!mbox.readBoxBuf(MBX_OFFSET_DATA, &smbdid)) return _MBEr_Fail;      			// get dat00, smbus device id
		if (!mbox.readBoxBuf(MBX_OFFSET_DATA + 1, &fanid)) return _MBEr_Fail;       		// get dat01, fan device id
...
int mbx_tml_writeDevice(uint8_t dev_no, uint8_t smbdid, uint8_t fanid, uint8_t *wdata, uint8_t *len)
{
    ...
    if (!mbox.writeBoxBuf(MBX_OFFSET_DATA, smbdid)) return _MBEr_Fail;     	//set dat00, smbus id
    if (!mbox.writeBoxBuf(MBX_OFFSET_DATA + 1, fanid)) return _MBEr_Fail;   //set dat01, fan id
    ...
    if (!mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_WR_THERMZONE)) return _MBEr_Fail;       	//set Cmd 0x43
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;               								//wait cmd clear
    if (!mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp)) return _MBEr_Fail;     					//read status
    if (u8tmp != _MBEr_Success) return u8tmp;                 //fail
