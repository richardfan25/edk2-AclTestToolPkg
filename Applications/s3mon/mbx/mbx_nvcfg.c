...
int mbx_nvcfg_readStatus(uint8_t *rstatus)
{
    ...
    if (!mbox.writeBoxBuf(MBX_OFFSET_PARA, PARA_NVCFG_RD_STS)) return _MBEr_Fail;   //set Para
    if (!mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_NVCFG)) return _MBEr_Fail;       	//set Cmd
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;               						//wait cmd clear
    if (!mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp)) return _MBEr_Fail;     			//read status
    if (u8tmp != _MBEr_Success) return u8tmp;                 						//fail
    if (!mbox.readBoxBuf(MBX_OFFSET_DATA, rstatus)) return _MBEr_Fail;    			//read dat00
...
int mbx_nvcfg_writeStatus(uint8_t wstatus)
{
    ...
    mbox.clearBoxBuf();
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;               						//wait cmd clear
    if (!mbox.writeBoxBuf(MBX_OFFSET_PARA, PARA_NVCFG_WR_STS)) return _MBEr_Fail;   //set Para
    if (!mbox.writeBoxBuf(MBX_OFFSET_DATA, wstatus)) return _MBEr_Fail;   			//set dat00
    if (!mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_NVCFG)) return _MBEr_Fail;       	//set Cmd
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;               						//wait cmd clear
    if (!mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp)) return _MBEr_Fail;     			//read status
    if (u8tmp != _MBEr_Success) return u8tmp;                 						//fail
...
int mbx_nvcfg_readControl(uint8_t *rstatus)
{
    ...
    mbox.clearBoxBuf();
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;               						//wait cmd clear
    if (!mbox.writeBoxBuf(MBX_OFFSET_PARA, PARA_NVCFG_RD_CTRL)) return _MBEr_Fail;  //set Para
    if (!mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_NVCFG)) return _MBEr_Fail;       	//set Cmd
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;               						//wait cmd clear
    if (!mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp)) return _MBEr_Fail;     			//read status
    if (u8tmp != _MBEr_Success) return u8tmp;                 						//fail
...
int mbx_nvcfg_writeControl(uint8_t wstatus)
{
    ...
    mbox.clearBoxBuf();
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;               						//wait cmd clear
    if (!mbox.writeBoxBuf(MBX_OFFSET_PARA, PARA_NVCFG_WR_CTRL)) return _MBEr_Fail;  //set Para
    if (!mbox.writeBoxBuf(MBX_OFFSET_DATA, wstatus)) return _MBEr_Fail;  			//set dat00
    if (!mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_NVCFG)) return _MBEr_Fail;       	//set Cmd
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;               						//wait cmd clear
    if (!mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp)) return _MBEr_Fail;     			//read status
...
int mbx_nvcfg_readBuf(uint8_t len, uint8_t *rdata, uint8_t offset)
{
    ...
    mbox.clearBoxBuf();
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;               						//wait cmd clear
    if (!mbox.writeBoxBuf(MBX_OFFSET_PARA,PARA_NVCFG_RD_CFGBUF)) return _MBEr_Fail; //set Para
    ...
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;               						//wait cmd clear
    if (!mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp)) return _MBEr_Fail;     			//read status
    if (u8tmp != _MBEr_Success) return u8tmp;    
...
int mbx_nvcfg_writeBuf(uint8_t len, uint8_t *wdata, uint8_t offset)
{
    ...
    mbox.clearBoxBuf();
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;               						//wait cmd clear
    if (!mbox.writeBoxBuf(MBX_OFFSET_PARA,PARA_NVCFG_WR_CFGBUF)) return _MBEr_Fail; //set Para
    if (!mbox.writeBoxBuf(MBX_OFFSET_DATA,offset)) return _MBEr_Fail;     			//set dat00
    if (!mbox.writeBoxBuf(MBX_OFFSET_DATA_END,len)) return _MBEr_Fail;        		//set dat2C
