 ...
 *===========================================================*/
UINT8 find_smbdev_addr(eDynclDevNo did, UINT8 buf_len, UINT8 *buf)
{
	int  ret;
	UINT8	wbuf[4];
	UINT8	rbuf[4];
 ...
 * Output: None
 *===========================================================*/
void print_smbdev_addr(UINT8 addr_cnt, UINT8 *addr_buf)
{
	UINT8	idx;
	UINT8	flag = 0;
	UINT8	len;
	...
	for (idx = 0; idx < mApp.dyna->size; idx++)
	{
		if((mApp.dyna->dev[idx].did != DDevNo_PECI) && (mApp.dyna->getDevType(mApp.dyna->dev[idx].did) == DDevType_SMB))
		{
			...
			len = find_smbdev_addr(mApp.dyna->dev[idx].did, addr_cnt, abuf);
			if(len == 0)
				continue;
			
			DevNo = mApp.dyna->dev[idx].pin_no & 0x07;
			if((flag & (0x01 << DevNo)) == 0)
			{
				flag |= (0x01 << DevNo);
				fprintf(stderr, "\nSMBus %d - ", DevNo);
				while(len > 0)
...
int test_smb_main(void)
{
	...
	while(1)
	{
		...
		fprintf(stderr, "[b]  Test SMBus0 - Read EERPOM          .    \n");
		fprintf(stderr, "[c]  Test SMBus0 - Write EERPOM         .    \n");
		fprintf(stderr, "[d]  Test SMBus0 - Auto Write EEPROM    .    \n");
		fprintf(stderr, "[e]  Test SMBus1 - Read Thermal IC      .    \n");
		fprintf(stderr, "[f]  Test SMBus1 - Write Thermal IC     .    \n");
		fprintf(stderr, "[g]  Test SMBus1 - Write DIO Board      .    \n");
		...
		{
			fprintf(stderr, "[SMBus 0 TEST] Read EEPROM\n\n");

			smb_addr = (UINT8) get_number_stdin("Enter EEPROM Address(0~0xFF):", 1, 2);

			test_smb_read_eeprom(DDevNo_SMBEEPROM, smb_addr);
		}
		else if(keyChar=='C' ||  keyChar=='c' )
		{
			fprintf(stderr, "[SMBus 0 TEST] Write EEPROM\n\n");

			smb_addr = (UINT8) get_number_stdin("Enter EEPROM Address(0~0xFF):", 1, 2);

			smb_data = (UINT8) get_number_stdin("Enter EEPROM value:", 1, 2);
							
			test_smb_write_eeprom(DDevNo_SMBEEPROM, smb_addr, smb_data, 0);
		}
		else if(keyChar=='D' ||  keyChar=='d' )
		{
			fprintf(stderr, "[SMBus 0 TEST] Write EEPROM With Auto Increase Data\n\n");

			smb_addr = (UINT8) get_number_stdin("Enter EEPROM Address(0~0xFF):", 1, 2);

			smb_data = (UINT8) get_number_stdin("Enter EEPROM value:", 1, 2);
									
			test_smb_write_eeprom(0x2B, smb_addr, smb_data, 1);
		}
		else if(keyChar=='E' ||  keyChar=='e' )
		{
			fprintf(stderr, "[SMBus 1 TEST] Read Thermal IC\n\n");
			
			fprintf(stderr, "Search for LM95245 please wait...\r");
			print_smbdev_addr(sizeof(lm95245_addr), lm95245_addr);

			smb_addr = (UINT8) get_number_stdin("Enter Thermal IC Address(0~0xFF):", 1, 2);
			
			tmic_list_reg_data(smb_addr);
		}
		else if(keyChar=='F' ||  keyChar=='f' )
		{
			fprintf(stderr, "[SMBus 1 TEST] Write Thermal IC\n\n");
			fprintf(stderr, "Search for LM95245 please wait...\r");
			print_smbdev_addr(sizeof(lm95245_addr), lm95245_addr);

			smb_addr = (UINT8) get_number_stdin("Enter Thermal IC Address(0~0xFF):", 1, 2);
			rtn = tmic_get_chip_id(smb_addr, &u16temp);
			if(rtn == _MBEr_Success)
			{
				...
				rtn = tmic_write_reg(smb_addr, LM_REG_T_CRIT_LMT, smb_data);
				rtn = tmic_read_reg(smb_addr, LM_REG_T_CRIT_LMT, &u8temp);
				if(rtn == _MBEr_Success)
				{
					if(smb_data == u8temp)
					{
		...
		{
			fprintf(stderr, "[SMBus 1 TEST] Write DIO Board\n\n");
			fprintf(stderr, "Search for PCA5555 please wait...\r");
			print_smbdev_addr(sizeof(pca9555_addr), pca9555_addr);
			...
			rtn = dio_write_reg(smb_addr, DIO_REG_CONFIG0, 0);
			if(rtn == _MBEr_Success)
			{
				// set new port0 level
				rtn = dio_write_reg(smb_addr, DIO_REG_OUT_PORT0, smb_data);
				if(rtn == _MBEr_Success)
				{
					// read port0 level
					rtn = dio_read_reg(smb_addr, DIO_REG_IN_PORT0, &u8temp);
					if(rtn == _MBEr_Success)
					{
						if(smb_data != u8temp)
							rtn = _MBEr_Fail;
