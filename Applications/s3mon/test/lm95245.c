 ...
 *===========================================================*/
int tmic_read_reg(UINT8 addr, UINT8 reg, UINT8 *data)
{
	UINT8		wbuf[4];
	UINT8		rbuf[4];
	UINT16		rlen;
 ...
 * Output: 1: operation success, Others: smbus error code
 *===========================================================*/
int tmic_write_reg(UINT8 addr, UINT8 reg, UINT8 data)
{
	UINT8		wbuf[4];
	UINT8		rbuf[4];
	UINT16		rlen;
 ...
 *===========================================================*/
int tmic_get_chip_id(UINT8 addr, UINT16 *id)
{
	int		ret = 0;
	UINT8	value;

	ret = tmic_read_reg(addr, LM_REG_MFU_ID, &value);
	...
	*id = (UINT16)value << 8;
	...
	ret = tmic_read_reg(addr, LM_REG_CHIP_REV, &value);
	if(ret != _MBEr_Success)
		return ret;
 ...
 * Output: 1: address is for TI lm95245, Others: error or is not for that
 *===========================================================*/
int tmic_list_reg_data(UINT8 addr)
{
	int		ret = 0;
	UINT16	u16data;
	UINT8	u8data;
	
	ret = tmic_get_chip_id(addr, &u16data);
	if(ret != _MBEr_Success)
		return ret;
	fprintf(stderr, "\nThermal Chip ID: 0x%04X\n", u16data);
	
	ret = tmic_read_reg(addr, LM_REG_TEMP_H, &u8data);
	if(ret != _MBEr_Success)
		return ret;
	fprintf(stderr, "Local Temp:  %3d C\n", u8data);
	
	ret = tmic_read_reg(addr, LM_REG_REMOTE_OFFSET_H, &u8data);
	if(ret != _MBEr_Success)
		return ret;
	fprintf(stderr, "Remote Temp: %3d C\n", u8data);
	
	
	ret = tmic_read_reg(addr, LM_REG_T_CRIT_LMT, &u8data);
	if(ret != _MBEr_Success)
		return ret;
	fprintf(stderr, "Local T_Crit Limit:  %3d C\n", u8data);

	ret = tmic_read_reg(addr, LM_REG_REMOTE_OS_LMT, &u8data);
	if(ret != _MBEr_Success)
		return ret;
	fprintf(stderr, "Remote OS Limit:     %3d C\n", u8data);
	
	ret = tmic_read_reg(addr, LM_REG_REMOTE_CRIT_LMT, &u8data);
	if(ret != _MBEr_Success)
		return ret;
	fprintf(stderr, "Remote T_Crit Limit: %3d C\n", u8data);
	
	ret = tmic_read_reg(addr, LM_REG_COMM_HYSTERESIS, &u8data);
	if(ret != _MBEr_Success)
		return ret;
	fprintf(stderr, "Common Hysteresis:    %2d C\n", u8data);
	
	ret = tmic_read_reg(addr, LM_REG_STATUS_1, &u8data);
	if(ret != _MBEr_Success)
		return ret;
