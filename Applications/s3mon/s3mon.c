...
void proc_mainMenu(int key)
{
	...
	ConsoleCurrRow++;
	gotoxy(0, ConsoleCurrRow);
	fprintf(stderr, "C. Simulate Power Button");
	ConsoleCurrRow++;
	gotoxy(0, ConsoleCurrRow);
	...
	else if ((keybyte == 'C')||(keybyte == 'c'))	//c C
	{
		menuSelect = _mid_simpwrbtn;
	}
...
void main(void)
{
	...
	fprintf(stderr, "\nSimulate Press Power Button\n");
	fprintf(stderr, "%s ver:%c%02X_%02X\n\n", project, vertype, major_Ver, minor_Ver);
