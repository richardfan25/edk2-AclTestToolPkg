...
//=============================================================================
uint32_t get_number_stdin(CHAR8 *msg, uint8_t type, uint8_t digi)
{
	int	  key;
	CHAR8 ch;
	CHAR8 *num_buf;
...
//======================================
//	parameter
//		type : 0 = 0~9 , other = 0~F
//		len : input length
//		value : output user key in value
//
//	return 0 = stop , 1 = success
//======================================
uint8_t UserKeyInValue(uint8_t type, uint8_t len, uint32_t *value)
{
	...
	if(len == 0)
		len = 1;
	else if(type == 0 && len > 10)
		len = 10;
	else if(type != 0 && len > 4)
		len = 4;
	
	num_buf = (CHAR8 *)malloc(len + 1);
	*num_buf = ' ';
	*(num_buf + 1) = '\0';
	...
	if(idx)
	{
		*value = str_to_dec(type, num_buf);
	}
	else
	{
		*value = 100;	//if no input any number, default 100
	}
