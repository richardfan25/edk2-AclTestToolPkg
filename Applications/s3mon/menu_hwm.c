    ...
    while (1)
    {
        if (mIsFlag1(doneFlag, hwmDone)) {
			...
			for (idx = 0; idx < mApp.tml->dev_cnt; idx++) {
				if (mApp.tml->dev[idx].addr == 0) continue;
			...
			ConsoleCurrRow++;
			for (idx = 0; idx < mApp.dyna->size; idx++) {
				if (mApp.dyna->dev[idx].type != DDevType_ADC) continue;
				if (mApp.dyna->getPinstate(mApp.dyna->dev[idx].did, u8tmp) != _MBEr_Success)
				{
					continue;
				}
...
void wdCldRstMenu(int key)
{
    ...
    if(mIsFlag1(doneFlag, wdIRQCldRstDone)) goto _wdCldRstMenu_end;
    clrscr();
    kdebug(0, 0);
    ConsoleCurrRow = 0;
    gotoxy(0, ConsoleCurrRow);
...
void wdWrmRstMenu(int key)
{
    ...
    if(mIsFlag1(doneFlag, wdIRQWrmRstDone)) goto _wdWrmRstMenu_end;
    clrscr();
    kdebug(0, 0);
    ConsoleCurrRow = 0;
    gotoxy(0, ConsoleCurrRow);
...
void wdWDPinMenu(int key)
{
    ...
    if(mIsFlag1(doneFlag, wdWDPinDone)) goto _wdWDPinMenu_end;
    clrscr();
    kdebug(0, 0);
    ConsoleCurrRow = 0;
    gotoxy(0, ConsoleCurrRow);
...
void sFanTestMenu(int key)
{
    ...
    if (mIsFlag1(doneFlag, sfanDone)) goto _sFanTestMenu_end;
    clrscr();
    kdebug(0, 0);
    ConsoleCurrRow = 0;
    gotoxy(0, ConsoleCurrRow);
    ...
    for (idx = 0; idx < mApp.fan->dev_cnt; idx++) {
        if (mApp.fan->dev[idx].code == 0) continue;
        ...
        if (mApp.dyna->getPinstate(mApp.fan->dev[idx].tacho_id, u8tmp) == _MBEr_Success)
        {
...
void gpioTestMenu(int key)
{
    ...
    if (mIsFlag1(doneFlag, gpioDone)) goto _gpioTestMenu_end;
    clrscr();
    kdebug(0, 0);
    ConsoleCurrRow = 0;
    gotoxy(0, ConsoleCurrRow);
...
void SimPowerButton(int key)
{
	uint8_t u8tmp;
	uint8_t time_log_str[100]={'\0'};
	uint32_t i, k_dtime, s_dtime, e_dtime, b_dtime;	//b_dtime : button active low time
	uint32_t detect_fall_count=0, detect_rise_count=0;
	uint32_t	keep_s4_high_cnt;

	//ambulance : lo_hz=650~750, hi_hz=900~1000, lo_duration=400ms, hi_duration=600ms
	uint16_t	lo = 1700;	// 700 Hz => 1190000/700 = 1700
	uint16_t	hi = 1253;	// 950 Hz => 1190000/950 = 1253
	uint8_t		val;
	uint8_t		err_beep;
	...
	gotoxy(0, ConsoleHeight - 1);
	fprintf(stderr, "Press ESC To End Test");

	ConsoleCurrRow = 0;
	gotoxy(0, ConsoleCurrRow);

	fprintf(stderr, "Please Enter T0 (seconds):");
	if(!UserKeyInValue(0, 7, &k_dtime))
	{
		goto _SimPowerButton_end;
	}

	fprintf(stderr, "Please Enter T1 (ms):");
	if(!UserKeyInValue(0, 7, &s_dtime))
	{
		goto _SimPowerButton_end;
	}

	fprintf(stderr, "\nPlease Enter T2 (ms):");
	if(!UserKeyInValue(0, 7, &e_dtime))
	{
		goto _SimPowerButton_end;
	}

	fprintf(stderr, "\nPlease Enter Power Button Hold Time (ms):");
	if(!UserKeyInValue(0, 7, &b_dtime))
	{
		goto _SimPowerButton_end;
	}

	//get time and print
	sprintf(time_log_str, "\nStart Test Time : %s \n", GetCurrentTime());
	fprintf(stderr, "%s", time_log_str);

	//write log
	InitFileSystem();
	sprintf(time_log_str, "%sT1 = %d ms,   T2 = %d ms,   Power Button Hold Time = %d ms\n", time_log_str, s_dtime, e_dtime, b_dtime);
	WriteLogFile(L"s3mon.txt", time_log_str);
	memset(time_log_str, '\0', strlen(time_log_str));

	//init gpio
	fprintf(stderr, "Init IO... \n");
	mApp.gpio->setSetting(DDevNo_AltGpio04, OUTPUT);
	mApp.gpio->setSetting(DDevNo_AltGpio06, INPUT);

	u8tmp = 1;
	mApp.dyna->setPinstate(DDevNo_AltGpio04, &u8tmp);
	gBS->Stall(100*1000);	//1sec
	fprintf(stderr, "Init IO Done \n");

	//get target IO init status
	mApp.dyna->getPinstate(DDevNo_AltGpio06, &u8tmp);

	//show init message
	ConsoleCurrRow = 10;
	gotoxy(0, ConsoleCurrRow);
	fprintf(stderr, "Stage 1 : Waiting Falling, Now Status : %s \n", u8tmp?"High":"Low");
	fprintf(stderr, "Stage 2 : Waiting %d ms (T1) \n", s_dtime);
	fprintf(stderr, "Stage 3 : Power Button Low Signal %d ms \n", b_dtime);
	fprintf(stderr, "Stage 4 : Power Button High Signal \n");
	fprintf(stderr, "Stage 5 : Waiting %d ms (T2) \n", e_dtime);
	fprintf(stderr, "Stage 6 : Detect Input Status");

	ConsoleCurrRow = 17;
	gotoxy(0, ConsoleCurrRow);
	fprintf(stderr, "Input Fall Count : %d \n", detect_fall_count);
	fprintf(stderr, "Input Rise Count : %d ", detect_rise_count);

	//check target IO is high
	while(!u8tmp)
	{
		//ESC to stop test
		if(CheckKeyIsESC())
		...
		fprintf(stderr, "Stage 1 : Waiting Falling, Now Status : %s ", u8tmp?"High":"Low");
	}
	...
	while(1)
	{
		...
		//detect S3 status,if S3 is Low,break loop
		ConsoleCurrRow = 10;
		gotoxy(0, ConsoleCurrRow);
		console_color(YELLOW, BLACK);
		fprintf(stderr, "Stage 1 : Waiting Falling, Now Status : %s ", u8tmp?"High":"Low");
		...
		while(1)
		{
			...
			if(!u8tmp)
			{
				...
				ConsoleCurrRow = 17;
				gotoxy(0, ConsoleCurrRow);
				console_color(WHITE, BLACK);
				fprintf(stderr, "Input Fall Count : %d ", detect_fall_count);
				break;
			}
			else
			{
				keep_s4_high_cnt++;	// 10ms
				if (keep_s4_high_cnt > k_dtime*100)
					err_beep = 1;
			}
			...
			if (err_beep)
			{
				...
				// freq : lo
				IoWrite8(0x43, 0xB6);
				IoWrite8(0x42, (uint8_t)(lo & 0xFF));
				IoWrite8(0x42, (uint8_t)((lo>>8) & 0xFF));
		
				gBS->Stall(400*1000);	// 400ms

				// freq : hi
				IoWrite8(0x43, 0xB6);
				IoWrite8(0x42, (uint8_t)(hi & 0xFF));
				IoWrite8(0x42, (uint8_t)((hi>>8) & 0xFF));
	
				gBS->Stall(600*1000);	// 600ms
		...
		sprintf(time_log_str, "%s Input Fall Count : %d\n", GetCurrentTime(), detect_fall_count);
		WriteLogFile(L"s3mon.txt", time_log_str);
		memset(time_log_str, '\0', strlen(time_log_str));

		//simulate power button, wait t1
		ConsoleCurrRow = 11;
		gotoxy(0, ConsoleCurrRow);
		console_color(YELLOW, BLACK);
		fprintf(stderr, "Stage 2 : Waiting %d ms (T1)", s_dtime);
		for(i=0; i<s_dtime; i++)
		{
			//ESC to stop test
			if((i%100) == 0)
		...
		//output low, wait Power Button Hold Time
		ConsoleCurrRow = 12;
		gotoxy(0, ConsoleCurrRow);
		console_color(YELLOW, BLACK);
		fprintf(stderr, "Stage 3 : Power Button Low Signal %d ms", b_dtime);
		u8tmp = 0;
		mApp.dyna->setPinstate(DDevNo_AltGpio04, &u8tmp);
		for(i=0; i<b_dtime; i++)
		{
		...
		gotoxy(0, ConsoleCurrRow);
		console_color(YELLOW, BLACK);
		fprintf(stderr, "Stage 5 : Waiting %d ms (T2)", e_dtime);
		for(i=0; i<e_dtime; i++)
		{
			//ESC to stop test
			if((i%100) == 0)
		...
		if(u8tmp)
		{
			...
			//get time and print
			sprintf(time_log_str, "%s TEST PASS!!", GetCurrentTime());
			fprintf(stderr, "%s", time_log_str);
			...
			//write log
			sprintf(time_log_str, "%s Input Rise Count : %d\n", time_log_str, detect_rise_count);
			WriteLogFile(L"s3mon.txt", time_log_str);
			memset(time_log_str, '\0', strlen(time_log_str));
		}
		else
		{
			...
			//get time and print
			sprintf(time_log_str, "%s TEST FAIL!!", GetCurrentTime());
			fprintf(stderr, "%s", time_log_str);
...
void lvdsOnOffTestMenu(int key)
{
    ...
    if (mIsFlag1(doneFlag, lvdsOnOffDone)) goto _lvdsOnOffTestMenu_end;
    clrscr();
    kdebug(0, 0);
    ConsoleCurrRow = 0;
    gotoxy(0, ConsoleCurrRow);
...
void lvdsLevelTestMenu(int key)
{
    ...
    if (mIsFlag1(doneFlag, lvdsLvlDone)) goto _lvdsLevelTestMenu_end;
    clrscr();
    kdebug(0, 0);
    ConsoleCurrRow = 0;
    gotoxy(0, ConsoleCurrRow);
    ...
    while (1)
    {
        ...
        if (diu8tmp == noValue)
        {
            ...
            for (idx = 0; idx < mApp.dyna->size; idx++)
            {
                ...
                {
                    if (mApp.dyna->dev[idx].type == DDevType_SMB) break;
                }
            }
        ...
        gotoxy(0, ConsoleCurrRow);
        fprintf(stderr, "Input protocol Byte/Word (1-word, other-byte): ");
        if (lenTemp == wordtype) fprintf(stderr, "Word.");
        else fprintf(stderr, "Byte.");
        ConsoleCurrRow++;
        //get rwtemp
        if (rwTemp == noValue)
        {
        ...
        gotoxy(0, ConsoleCurrRow);
        fprintf(stderr, "Input read/write (1-write, other-read): ");
        if (rwTemp == writetype) fprintf(stderr, "Write.");
        else fprintf(stderr, "Read.");
        ConsoleCurrRow++;
        //get write data
        if (rwTemp == writetype)
        {
