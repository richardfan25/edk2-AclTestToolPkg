...
int mbx_dyna_getPinstate(eDynclDevNo did, uint8_t *idata)
{
    ...
    if (!mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_RD_HWPIN)) return _MBEr_Fail; //set CMD 0x11
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;            						//wait cmd clear
...
int mbx_dyna_setPinstate(eDynclDevNo did, uint8_t *odata)
{
    ...
    case DDevType_PWM:
    case DDevType_DAC:
        if (!mbox.writeBoxBuf(MBX_OFFSET_DATA, odata[0])) return _MBEr_Fail; 	//set Dat00
        break;
    case DDevType_ADC:
    case DDevType_TACHO:
    default:
    ...
    if (!mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_WR_HWPIN)) return _MBEr_Fail;	//set CMD 0x12 write gpio command
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;            						//wait cmd clear
    if (!mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp)) return _MBEr_Fail;  			//read status
...
int mbx_dyna_getTable(void)
{
    ...
    //read device id
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;           							//wait cmd clear
    if (!mbox.writeBoxBuf(MBX_OFFSET_PARA, PARA_DYNATBL_DID)) return _MBEr_Fail;   	//set PAPA 0, read device id
    if (!mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_DYNATBL)) return _MBEr_Fail;   	//set CMD 0x20
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;           							//wait cmd clear
    if (!mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp)) return _MBEr_Fail; 				//read status
    if (u8tmp != _MBEr_Success) return u8tmp;             //fail
    ...
    //read HW pin code
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;           							//wait cmd clear
    if (!mbox.writeBoxBuf(MBX_OFFSET_PARA, PARA_DYNATBL_HWPIN)) return _MBEr_Fail;  //set PAPA 1, read HW pin
    if (!mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_DYNATBL)) return _MBEr_Fail;   	//set CMD 0x20
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;           							//wait cmd clear
    if (!mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp)) return _MBEr_Fail; 				//read status
    if (u8tmp != _MBEr_Success) return u8tmp;             //fail
    ...
    //read polarity
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;           							//wait cmd clear
    if (!mbox.writeBoxBuf(MBX_OFFSET_PARA, PARA_DYNATBL_ACTIVE)) return _MBEr_Fail; //set PAPA 2, read polarity
    if (!mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_DYNATBL)) return _MBEr_Fail;   	//set CMD 0x20
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;           							//wait cmd clear
    if (!mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp)) return _MBEr_Fail; 				//read status
    if (u8tmp != _MBEr_Success) return u8tmp;             //fail
