...
int mbx_wdt_start()
{
    ...
    if (!mbox.writeBoxBuf(MBX_OFFSET_PARA, PARA_WDT_START)) return _MBEr_Fail;  //set Para
    if (!mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_WDT)) return _MBEr_Fail;      //set Cmd
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;                					//wait cmd clear
...
int mbx_wdt_stop()
{
    ...
    mbox.clearBoxBuf();
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;                					//wait cmd clear
    if (!mbox.writeBoxBuf(MBX_OFFSET_PARA, PARA_WDT_STOP)) return _MBEr_Fail;   //set Para
    if (!mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_WDT)) return _MBEr_Fail;      //set Cmd
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;                					//wait cmd clear
    if (!mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp)) return _MBEr_Fail;      		//read status
    if (u8tmp != _MBEr_Success) return u8tmp;                      				//fail
...
int mbx_wdt_reset(uint8_t *r_status)
{
    ...
    mbox.clearBoxBuf();
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;                					//wait cmd clear
    if (!mbox.writeBoxBuf(MBX_OFFSET_PARA, PARA_WDT_RESET)) return _MBEr_Fail;  //set Para
    if (!mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_WDT)) return _MBEr_Fail;      //set Cmd
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;                					//wait cmd clear
    if (!mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp)) return _MBEr_Fail;      		//read status
    if (u8tmp != _MBEr_Success) return u8tmp;                      				//fail
    if (!mbox.readBoxBuf(MBX_OFFSET_DATA, r_status)) return _MBEr_Fail;    		//read dat00
...
int mbx_wdt_readLimit(uint8_t *r_status, uint16_t *limit)
{
    ...
    mbox.clearBoxBuf();
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;                					//wait cmd clear
    if (!mbox.writeBoxBuf(MBX_OFFSET_PARA, PARA_WDT_RD_LIMIT)) return _MBEr_Fail;	//set Para
    if (!mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_WDT)) return _MBEr_Fail;      //set Cmd
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;                					//wait cmd clear
    if (!mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp)) return _MBEr_Fail;      		//read status
    if (u8tmp != _MBEr_Success) return u8tmp;                     				//fail
...
int mbx_wdt_writeLimit(uint8_t *r_status, uint16_t *limit)
{
    ...
    mbox.clearBoxBuf();
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;                						//wait cmd clear
    if (!mbox.writeBoxBuf(MBX_OFFSET_PARA, PARA_WDT_WR_LIMIT)) return _MBEr_Fail;   //set Para

    for (idx = 0; idx < WDT_LIMIT_SIZE; idx++)    									//setup limit data
    {
        u8tmp = (limit[idx] >> 8 & 0xFF);  //hi byte
        if (!mbox.writeBoxBuf(idx * 2 + MBX_OFFSET_DATA + 1, u8tmp)) return _MBEr_Fail;
        u8tmp = (limit[idx] & 0xFF);     //low byte
        if (!mbox.writeBoxBuf(idx * 2 + MBX_OFFSET_DATA + 2, u8tmp)) return _MBEr_Fail;
    }
    if (!mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_WDT)) return _MBEr_Fail;        	//set Cmd
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;                						//wait cmd clear
    if (!mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp)) return _MBEr_Fail;      			//read status
...
int mbx_wdt_bootStop()
{
    ...
    mbox.clearBoxBuf();
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;                						//wait cmd clear
    if (!mbox.writeBoxBuf(MBX_OFFSET_PARA, PARA_WDT_STOP_BOOT)) return _MBEr_Fail;  //set Para
    if (!mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_WDT)) return _MBEr_Fail;        	//set Cmd
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;                						//wait cmd clear
    if (!mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp)) return _MBEr_Fail;      			//read status
    if (!mbox.waitBoxBusy()) return _MBEr_Fail;                						//wait cmd clear
