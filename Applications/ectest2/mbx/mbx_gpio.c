...
int mbx_gpio_getSetting(uint8_t did, uint8_t *idata)
{
    ...
    if (mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_RD_GPIO_CFG) != _err_noerror)
		return _MBEr_Fail;  //set CMD 0x30
    if (mbox.waitBoxBusy() != _err_noerror)
		return _MBEr_Fail;            							//wait cmd clear
    ...
    {
        if (mbox.readBoxBuf(MBX_OFFSET_DATA, idata) != _err_noerror)
			return _MBEr_Success;			//read DAT0
        else
			return _MBEr_Fail;
    }
...
int mbx_gpio_setSetting(uint8_t did, uint8_t odata)
{
    ...
    if (mbox.waitBoxBusy() != _err_noerror)
		return _MBEr_Fail;            							//wait cmd clear
    if (mbox.writeBoxBuf(MBX_OFFSET_PARA,did) != _err_noerror)
		return _MBEr_Fail;     				//set PAPA 0, oemgpio
    if (mbox.writeBoxBuf(MBX_OFFSET_DATA,odata) != _err_noerror)
		return _MBEr_Fail;   				//write data into Dat00
    if (mbox.writeBoxBuf(MBX_OFFSET_CMD,MBX_CMD_WR_GPIO_CFG) != _err_noerror)
		return _MBEr_Fail;   //set CMD 0x31
    if (mbox.waitBoxBusy() != _err_noerror)
		return _MBEr_Fail;           							//wait cmd clear
    if (mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp) != _err_noerror)
		return _MBEr_Fail;  				//read status
