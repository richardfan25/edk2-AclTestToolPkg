...
int mbx_tpro_updateDevice(void)
{
	...
	for(tpro_no = 0; tpro_no < mbx_tpro.dev_cnt; tpro_no++)
	{
		...
		if (mbox.writeBoxBuf(MBX_OFFSET_PARA, tpro_no) != _err_noerror)
			return _MBEr_Fail;       		//set PAPA, zone number
		if (mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_RD_THERMPRO) != _err_noerror)
			return _MBEr_Fail;  //set Cmd 0x44
		if (mbox.waitBoxBusy() != _err_noerror)
			return _MBEr_Fail;               						//wait cmd clear
...
int mbx_tpro_writeDevice(uint8_t dev_no, uint8_t *wdata, uint8_t *len)
{
    ...
    if (mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_WR_THERMPRO) != _err_noerror)
		return _MBEr_Fail;      //set Cmd 0x45
    if (mbox.waitBoxBusy() != _err_noerror)
		return _MBEr_Fail;               							//wait cmd clear
    if (mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp) != _err_noerror)
		return _MBEr_Fail;     				//read status
