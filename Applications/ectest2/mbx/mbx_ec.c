...
int mbx_ec_readEcRam(uint8_t bank, uint8_t offset, uint8_t len, uint8_t *rdata)
{
    ...
    if (mbox.writeBoxBuf(MBX_OFFSET_PARA, bank) != _err_noerror)
		return _MBEr_Fail;        	//set PAPA
    ...
    if (mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_RD_ECRAM) != _err_noerror)
		return _MBEr_Fail; //set Cmd 0x1E
    if (mbox.waitBoxBusy() != _err_noerror)
		return _MBEr_Fail;                					//wait cmd clear
...
int mbx_ec_writeEcRam(uint8_t bank, uint8_t offset, uint8_t len, uint8_t *wdata)
{
    ...
    if (mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_WR_ECRAM) != _err_noerror)
		return _MBEr_Fail; //set Cmd 0x1F
    if (mbox.waitBoxBusy() != _err_noerror)
		return _MBEr_Fail;                					//wait cmd clear
    if (mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp) != _err_noerror)
		return _MBEr_Fail;      		//read status
...
int mbx_ec_readBufRam(uint8_t *rdata, uint16_t len)
{
    ...
    {
        if (mbox.writeBoxBuf(MBX_OFFSET_PARA, idx) != _err_noerror)
			return _MBEr_Fail;      			//set PAPA 0, bank number
        if (mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_RD_BUFRAM) != _err_noerror)
			return _MBEr_Fail;    //set cmd 0xC1
        if (mbox.waitBoxBusy() != _err_noerror)
			return _MBEr_Fail;                						//wait cmd clear
        if (mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp) != _err_noerror)
			return _MBEr_Fail;      			//read status
...
int mbx_ec_writeBufRam(uint8_t *wdata, uint16_t len)
{
    ...
    {
        if (mbox.writeBoxBuf(MBX_OFFSET_PARA, idx) != _err_noerror)
			return _MBEr_Fail;							//set PAPA, bank number
        ...
        if (mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_WR_BUFRAM) != _err_noerror)
			return _MBEr_Fail;        	//set cmd 0xC2
        if (mbox.waitBoxBusy() != _err_noerror)
			return _MBEr_Fail;                								//wait cmd clear
        if (mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp) != _err_noerror)
			return _MBEr_Fail;      					//read status
...
int mbx_ec_readFwInfo(char *prjname, char *version)
{
    ...
    if (mbox.waitBoxBusy() != _err_noerror)
		return _MBEr_Fail;           						//wait cmd clear
    if (mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_VERSION) != _err_noerror)
		return _MBEr_Fail;  //set CMD 0xF0
    if (mbox.waitBoxBusy() != _err_noerror)
		return _MBEr_Fail;           						//wait cmd clear
    if (mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp) != _err_noerror)
		return _MBEr_Fail; 			//read status
