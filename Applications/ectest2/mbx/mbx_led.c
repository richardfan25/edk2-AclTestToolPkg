...
int mbx_led_updateDevice(uint8_t dev_no, uint8_t* gpioid, stLedDev *rdata, uint8_t *len)
{
    ...
    if (mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_RD_LED) != _err_noerror)
		return _MBEr_Fail;   //set Cmd 0x46
    if (mbox.waitBoxBusy() != _err_noerror)
		return _MBEr_Fail;               					//wait cmd clear
    ...
    if (u8tmp != _MBEr_Success)
		return u8tmp;                 					//fail
    if (mbox.readBoxBuf(MBX_OFFSET_DATA, gpioid) != _err_noerror)
		return _MBEr_Fail;     		// get dat00, gpio device id
    ...
    *len = u8tmp;
    ...
    if (mbox.readBoxBuf(MBX_OFFSET_DATA + 1, &u8tmp) != _err_noerror)
		return _MBEr_Fail;     	//read dat01, pin
    rdata->pin = u8tmp;
    if (mbox.readBoxBuf(MBX_OFFSET_DATA + 2, &u8tmp) != _err_noerror)
		return _MBEr_Fail;     	//read dat02, control hi
    rdata->control = (u8tmp << 8);
    if (mbox.readBoxBuf(MBX_OFFSET_DATA + 3, &u8tmp) != _err_noerror)
		return _MBEr_Fail;     	//read dat03, control low
    rdata->control |= u8tmp;
    if (mbox.readBoxBuf(MBX_OFFSET_DATA + 4, &u8tmp) != _err_noerror)
		return _MBEr_Fail;     	//read dat04, behavior
...
int mbx_led_writeDevice(uint8_t dev_no, uint8_t gpioid, stLedDev *wdata, uint8_t *len)
{
    ...
    if (mbox.writeBoxBuf(MBX_OFFSET_PARA, dev_no) != _err_noerror)
		return _MBEr_Fail;       	//set PAPA, zone number
    if (mbox.writeBoxBuf(MBX_OFFSET_DATA, gpioid) != _err_noerror)
    ...
    u8tmp = wdata->pin;
    if (mbox.writeBoxBuf(MBX_OFFSET_DATA + 1, u8tmp) != _err_noerror)
		return _MBEr_Fail;     	//write dat01, pin
    u8tmp = (uint8_t)(wdata->control >> 8);
    if (mbox.writeBoxBuf(MBX_OFFSET_DATA + 2, u8tmp) != _err_noerror)
		return _MBEr_Fail;     	//write dat02, control hi
    u8tmp = (uint8_t)(wdata->control & 0xFF);
    if (mbox.writeBoxBuf(MBX_OFFSET_DATA + 3, u8tmp) != _err_noerror)
		return _MBEr_Fail;     	//write dat03, control low
    u8tmp = wdata->behavior;
    if (mbox.writeBoxBuf(MBX_OFFSET_DATA + 4, u8tmp) != _err_noerror)
		return _MBEr_Fail;     	//write dat04, behavior
    if (mbox.writeBoxBuf(MBX_OFFSET_CMD, MBX_CMD_WR_LED) != _err_noerror)
		return _MBEr_Fail;   //set Cmd 0x47
    if (mbox.waitBoxBusy() != _err_noerror)
		return _MBEr_Fail;               					//wait cmd clear
    if (mbox.readBoxBuf(MBX_OFFSET_STA, &u8tmp) != _err_noerror)
		return _MBEr_Fail;     		//read status
