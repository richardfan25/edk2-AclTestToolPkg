        ...
        if (mIsFlag1(autoFlags, startFlag) && mIsFlag1(autoFlags, fileFlag))
        {
            ...
            else if (step == _as_fan)
            {
                if (cnttemp == 1)
                {
					...
					if(isNewStuckVer)
					{
						if (mApp.fan->dev[fanid].fan_id == 0)
						{
							step	= _as_gpio;
							fanid 	= 0;
							cnttemp = 0;
						...
						itemp = mApp.fan->writeDevice(
								...
								mApp.fan->dev[fanid].speedDviceID,
								u8tmp,
								&lentemp);
					}
					else
                ...
                else if (cnttemp == fanOffDelayCnt)
                {
					if(isNewStuckVer)
					{
						if (mApp.dyna->getPinstate(mApp.fan->dev[fanid].speedDviceID, u8tmp) == _MBEr_Success)
						{
							itemp = u8tmp[0];
