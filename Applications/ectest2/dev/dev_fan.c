...
void fan_clearData(uint8_t dev_no)
{
	...
	fandev[dev_no].reserve0		= 0;
	fandev[dev_no].speedDviceID	= 0;
	fandev[dev_no].tempID		= 0;
}
 ...
 * Output: NA
 *===========================================================*/
void fan_updatData2(uint8_t dev_no, uint8_t *rdata, uint8_t len)
{
    uint8_t idx = 0;
    ...
    fandev[dev_no].fan_id	= rdata[idx];
    ...
    if (idx >= len)
		return;
    fandev[dev_no].Speed_Hi = rdata[idx];
	
    idx++;
    if (idx >= len)
		return;
    fandev[dev_no].reserve0 = rdata[idx];
	
    idx++;
    if (idx >= len)
		return;
    fandev[dev_no].Speed_Lo = rdata[idx];
	
    idx++;
    if (idx >= len)
		return;
    fandev[dev_no].speedDviceID = rdata[idx];
    ...
    if (idx >= len)
		return;
    fandev[dev_no].speedCtrlMode = rdata[idx];
	
    idx++;
    if (idx >= len)
		return;
    fandev[dev_no].tempID = rdata[idx];
}
/*===========================================================
 * Name  : toArray
 * Pupose: to array
 ...
 * Output: NA
 *===========================================================*/
void fan_toArray2(uint8_t dev_no, uint8_t *odata, uint8_t *len)
{
	if (dev_no >= FAN_CH_MAX) return;

    *len = 16;
    odata[0] = fandev[dev_no].fan_id;
    odata[1] = fandev[dev_no].status;
    odata[2] = fandev[dev_no].ctrlMode;
    odata[3] = fandev[dev_no].tempThresH;
    odata[4] = fandev[dev_no].tempThresL;
    ...
    odata[6] = fandev[dev_no].PWM_Hi;
    odata[7] = fandev[dev_no].PWM_Lo;
    odata[8] = (uint8_t)fandev[dev_no].Speed_Hi;
	odata[9] = fandev[dev_no].reserve0;
    odata[10] = (uint8_t)fandev[dev_no].Speed_Lo;
	odata[11] = fandev[dev_no].speedDviceID;
    odata[12] = fandev[dev_no].debounce;
    odata[13] = fandev[dev_no].tempCtrlMode;
    odata[14] = (uint8_t)fandev[dev_no].speedCtrlMode;
    odata[15] = fandev[dev_no].tempID;
}
/*===========================================================
 * Name  : getCtrlMode
